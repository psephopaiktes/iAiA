<template><PageView>
    ...
    <h2>表示名の変更</h2>
    <p>表示名を変更できます。表示名は、複数人プレイ時に他人に表示されるあなたのプレイヤー名です。</p>
    <p>現在の表示名: <b>{{ $store.state.user.displayName }}</b></p>
    <input class="inputText mt-16" type="text" v-model="newName" placeholder="新しい表示名を入力してください。">
    ...
    <h2>アカウント削除</h2>
    <p>アカウントを削除します。</p>
    <p>アカウントを削除すると、キャラクター情報などこれまでのデータもすべて消去されます。元には戻せませんのでご注意ください。</p>
...
export default class Account extends Vue {
  ...
  // lifecycle hook
  public beforeCreate() {
    if ( !this.$store.state.login ) { // 未ログインの場合
