...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    diceRoll(state, arg) {
      ...
      state.dice.result = throwDice(state.dice.n, state.dice.ub) + state.dice.add;
      state.dice.showModal = true;
      const logTextDice = (state.dice.add === 0)
        ? state.dice.n + 'D' + state.dice.ub
        : state.dice.n + 'D' + state.dice.ub + '+' + state.dice.add;
      const format = { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' };
      const logTextDate = Intl.DateTimeFormat('ja-JP', format).format( new Date() );
      const logItem: DiceItem = {
        dice: logTextDice,
        date: logTextDate,
      };
      state.dice.log.unshift( logItem );
      if (state.dice.log.length > 10) { state.dice.log.pop(); }
